...
internal object ActivityDetailsParser {
    ...
    fun parseDetails(html: String, currentYear: Int): ActivityDetails {
        ...
        return if (buttonBook.hasAttr("data-book-success")) {
            ...
            val data = jsonSerializer.decodeFromString<ActivityCancelDataJson>(json)
            ActivityDetails(
                name = cleanActivityFreetrainingName(data.`class`.name),
                dateTimeRange = dateRange,
                venueName = data.venue.name.trim(),
    ...
    private fun parseSlugFromGoogleMapUrls(jsonString: String): String {
        val url = jsonSerializer.decodeFromString<List<GoogleMapUrl>>(jsonString).first().url
...
internal fun extractDateTime(string: String): LocalDateTime {
