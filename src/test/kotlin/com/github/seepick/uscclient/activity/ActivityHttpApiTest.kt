...
class ActivityHttpApiTest : StringSpec() {
    ...
    init {
        "Given data returned When fetch page Then return data" {
            ...
            val expectedUrl = "/activities?city_id=${filter.city.id}&date=${
                filter.date.format(DateTimeFormatter.ISO_LOCAL_DATE)
            }&plan_type=${filter.plan.id}&type%5B%5D=${ActivityType.OnSite.apiValue}&service_type=${ServiceType.Courses.apiValue}&page=1"
            val http =
